...
class FtpHelper(object):
    ...
    def listdir(self, path):
        try:
            ...
            raise ExecuteException('', 'error while list dir from ftp server. %s' % error_perm.message)
        finally:
            self.ftp.quit()
    ...
    def delete_dir(self, target):
        try:
            ...
            raise ExecuteException('', 'error while delete dir from ftp server. %s' % error_perm.message)
        finally:
            self.ftp.quit()
    ...
    def upload_dir(self, source_file_dir, target):
        try:
            ...
            # traceback.print_exc()
            raise ExecuteException('', 'error while upload dir from ftp server. %s' % error_perm.message)
        finally:
            self.ftp.quit()
