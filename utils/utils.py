    ...
    for i in range(30):
        try:
            output = runCmdAndGetOutput(
                'kubectl get vmd --kubeconfig=/root/.kube/config -o=jsonpath="{range .items[?(@.spec.volume.poolname==\\"%s\\")]}{.metadata.name}{\\"\\t\\"}{.metadata.labels.host}{\\"\\n\\"}{end}"' % poolname)
            break
        except Exception:
            logger.debug(traceback.format_exc())
    disks = []
...
def get_pool_all_ss(poolname):
    ...
    for i in range(30):
        try:
            output = runCmdAndGetOutput(
                'kubectl get vmdsn --kubeconfig=/root/.kube/config -o=jsonpath="{range .items[?(@.spec.volume.poolname==\\"%s\\")]}{.metadata.name}{\\"\\t\\"}{.metadata.labels.host}{\\"\\n\\"}{end}"' % poolname)
            break
        except Exception:
            logger.debug(traceback.format_exc())
    disks = []
...
def get_pools_by_node(node_name):
    ...
    for i in range(30):
        try:
            output = runCmdAndGetOutput(
                'kubectl get vmp  --kubeconfig=/root/.kube/config -o=jsonpath="{range .items[?(.metadata.labels.host==\\"%s\\")]}{.metadata.name}{\\"\\t\\"}{.spec.pool.poolname}{\\"\\t\\"}{.metadata.labels.host}{\\"\\n\\"}{end}"' % node_name)
            break
        except Exception:
            logger.debug(traceback.format_exc())
...
def get_pools_by_poolname(poolname):
    ...
    for i in range(30):
        try:
            output = runCmdAndGetOutput(
                'kubectl get vmp  --kubeconfig=/root/.kube/config -o=jsonpath="{range .items[?(@.spec.pool.poolname==\\"%s\\")]}{.metadata.name}{\\"\\t\\"}{.metadata.labels.host}{\\"\\t\\"}{.spec.pool.path}{\\"\\n\\"}{end}"' % poolname)
            break
        except Exception:
            logger.debug(traceback.format_exc())
    pools = []
...
#         try:
#             runCmd('kubectl config set-cluster /root/.kube/config')
#             time.sleep(3)
...
def apply_all_jsondict(jsondicts):
    ...
    logger.debug(jsondicts)
    for i in range(len(jsondicts)):
        jsondict = jsondicts[i]
        rk = jsondict['kind']
        rname = jsondict['metadata']['name']
        helper = K8sHelper(rk)
        helper.updateAll(rname, jsondict)
...
def create_all_jsondict(jsondicts):
    ...
    for i in range(len(jsondicts)):
        ...
        rname = jsondict['metadata']['name']
        helper = K8sHelper(rk)
        helper.createAll(rname, jsondict)
