...
def migrate(params):
    ...
    if not is_vm_exist(params.domain) and vmHelper.exist(params.domain):
        success_print("vm disk not exist in this node, migrate vm disk %s successful." % params.domain, {})
    ...
    for i in range(30):
        try:
            output = rpcCallAndGetOutput(
                'kubectl get vmd --kubeconfig=/root/.kube/config -o=jsonpath="{range .items[?(@.metadata.name==\\"%s\\")]}{.spec.volume.poolname}{\\"\\t\\"}{.spec.volume.disk}{\\"\\t\\"}{.spec.volume.uni}{\\"\\t\\"}{.spec.nodeName}{\\"\\n\\"}{end}"' % uuid)
            break
        except Exception, e:
            pass
...
def get_disk_prepare_info_by_path(path):
    ...
    for i in range(30):
        try:
            if not success:
                output = rpcCallAndGetOutput(
                    'kubectl get vmd --kubeconfig=/root/.kube/config -o=jsonpath="{range .items[?(@.spec.volume.filename==\\"%s\\")]}{.spec.volume.poolname}{\\"\\t\\"}{.spec.volume.disk}{\\"\\t\\"}{.spec.volume.uni}{\\"\\t\\"}{.spec.nodeName}{\\"\\t\\"}{.spec.volume.pool}{\\"\\n\\"}{end}"' % path)
                if output and len(output.splitlines()) == 1 and len(output.splitlines()[0].split()) == 5:
                    success = True
            if not success:
                output = rpcCallAndGetOutput(
                    'kubectl get vmdsn --kubeconfig=/root/.kube/config -o=jsonpath="{range .items[?(@.spec.volume.filename==\\"%s\\")]}{.spec.volume.poolname}{\\"\\t\\"}{.spec.volume.disk}{\\"\\t\\"}{.spec.volume.uni}{\\"\\t\\"}{.spec.nodeName}{\\"\\t\\"}{.spec.volume.pool}{\\"\\n\\"}{end}"' % path)
                if output and len(output.splitlines()) == 1 and len(output.splitlines()[0].split()) == 5:
                    success = True
            if not success:
                output = rpcCallAndGetOutput(
                    'kubectl get vmdi --kubeconfig=/root/.kube/config -o=jsonpath="{range .items[?(@.spec.volume.filename==\\"%s\\")]}{.spec.volume.poolname}{\\"\\t\\"}{.spec.volume.disk}{\\"\\t\\"}{.spec.volume.uni}{\\"\\t\\"}{.spec.nodeName}{\\"\\t\\"}{.spec.volume.pool}{\\"\\n\\"}{end}"' % path)
                if output and len(output.splitlines()) == 1 and len(output.splitlines()[0].split()) == 5:
                    success = True
            break
        except Exception:
